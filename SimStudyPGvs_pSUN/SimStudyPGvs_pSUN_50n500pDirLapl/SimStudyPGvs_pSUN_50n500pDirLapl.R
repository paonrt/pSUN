  ...
  cl = cluster, x = matrix(1:niter), fun = function(xapp) {
    ...
    while (toDo) {
      ...
      pSUN_output = tryCatch(
        expr = pSUN::pSUN_LogitDirLapl_Gibbs(
...
################################### Closure ###################################
save(list = ls(), file = "SimStudyPGvs_pSUN_50n500pDirLapl.RData")