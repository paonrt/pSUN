#' @title Logit Model with Independent Bessel Priors: Polya-Gamma Gibbs Sampler

#' @description
#' Polya-Gamma Gibbs sampler for the logit model with independent Bessel priors.
#'  It is used the code from package \pkg{BayesLogit} with slight
#'  modifications.

#' @usage
#' logitBessitPG(nsim, X, y, nu, mu, Sigma, x0 = NULL, burn = 1,
#'               thin = 1, verbose = +Inf)

#' @details
#' \code{nu} is degrees of freedom for Bessel distributions. If \code{nu == 1}
#'  then the priors are Laplace distributions. If \code{nu} goes to infinity
#'  then the priors converge to Gaussian distributions.
#'
...
logitBessitPG = function(
  ...
  for (iburn in 1:burn) {
    ...
    ### update W
    W = apply(
      matrix((beta - mu)^2 / diagSigma), 1, FUN = function(xappl) {
        tryCatch(
    ...
    for (ithin in 1:thin) {
      ...
      W = apply(
        matrix((beta - mu)^2 / diagSigma), 1, FUN = function(xappl) {
          tryCatch(
            expr = GIGrvg::rgig(
              n = 1, lambda = 0.5 * nu, chi = xappl, psi = nu
            ),
            error = function(err){
              GIGrvg::rgig(
                n = 1, lambda = 0.5 * nu,
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitBessitPG: end time at ", Sys.time(), sep = ""))
  }
