#' @title Logit Model with Gaussian Prior: Variables Selection
...
#' @usage
#' LSBR_LogitGauss_VarSel = function(nsim, X, y, mu, Sigma, ntim = 1e+4,
#'                                   x0 = NULL, verbose = +Inf)
...
#'  density is computed by using the importance sampling described in Onorati
#'  and Liseo (2022).
#'
...
#' @export
LSBR_LogitGauss_VarSel = function(
  nsim, X, y, mu, Sigma, ntim = 1e+4, x0 = NULL, verbose = +Inf
) {

  ### print start time if required
  if (!is.infinite(verbose)) {
    print(paste("LSBR_LogitGauss_VarSel: start time at ", Sys.time(), sep = ""))
  }
  ...
  if (is.null(x0)) {
    currMod = logical(p)
    currMod[1] = TRUE
    x0 = pSUN_LogitGauss_IS(ntim, A = A, b = b, xi = xi, Omega = Omega)
    for (i in 2:p) {
      credInt = weightedQuantile(
        x0$sample[i, ], w = x0$normWeights, probs = c(0.025, 0.975)
      )
  ...
  logPostCurrMod = pSUN_LogitGauss_IS(
    ntim,
  ...
  for (insim in 1:nsim) {
    ...
    for (imetro in 2:p) {
      ...
      ### compute logarithm of posterior density
      logPostPropMod = pSUN_LogitGauss_IS(
        ntim,
        A = matrix(A[, propMod], ncol = propModSize),
        b = b, xi = xi[propMod],
  ...
  if (!is.infinite(verbose)) {
    print(paste("LSBR_LogitGauss_VarSel: end time at ", Sys.time(), sep = ""))
  }
