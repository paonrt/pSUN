#' @title Logit Model with Independent Bessel Priors: Ultimate Polya-Gamma
#'  Gibbs Sampler

#' @description
#' Ultimate Polya-Gamma Gibbs sampler for the logit model with independent
#'  Bessel priors. It is used the code from package \pkg{UPG} with slight
#'  modifications.

#' @usage
#' logitBessitUPG = function(nsim, X, y, nu, Sigma, x0 = NULL, burn = 1,
#'                           thin = 1, verbose = +Inf)
...
#' }
#'
#' The algorithm described in Zens et al. (2023) is used. The mixture density
...
logitBessitUPG = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitBessitUPG: start time at ", Sys.time(), sep = ""))
  }
  ...
  for (iburn in 1:burn) {
    ...
    ### update W
    W = apply(
      matrix(beta^2 / diagSigma), 1, FUN = function(xappl) {
        tryCatch(
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitBessitUPG: end time at ", Sys.time(), sep = ""))
  }
