...
#' @usage
#' pSUN_ProbitBessit_Gibbs = function(nsim, A, b, nu, xi, Omega, x0 = NULL,
#'                                    burn = 1, thin = 1, plim = 1 / nsim,
#'                                    verbose = +Inf)

#' @details
#' It is said \eqn{X} follows a pSUN probit independent Bessels distribution if
#'  \deqn{
...
#' @import truncnorm
#' @export
pSUN_ProbitBessit_Gibbs = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste(
      "pSUN_ProbitBessit_Gibbs: start time at ", Sys.time(), sep = ""
    ))
  }
  ...
  if (!is.infinite(verbose)) {
    print(paste("pSUN_ProbitBessit_Gibbs: end time at ", Sys.time(), sep = ""))
  }
