#' @title Perturbed Unified Skew Normal Logit Independent Bessel: Gibbs Sampler

#' @description
#' Gibbs Sampler for a perturbed unified skew normal (pSUN) logit indipendent
#'  Bessels distribution.

#' @usage
#' pSUN_LogitBessit_Gibbs = function(nsim, A, b, nu, xi, Omega, x0 = NULL,
#'                                   burn = 1, thin = 1, plim = 1 / nsim,
#'                                   verbose = +Inf)

#' @details
#' It is said \eqn{X} follows a pSUN logit independent Bessels distribution if
#'  \deqn{
#'   X = \xi + \mathrm{diag}^\frac{1}{2} (\Omega) \, Z \vert T \le A Z + b \\
#'   Z \vert W \sim N(0, \bar{\Omega}_W) \perp\!\!\!\perp T \vert V \sim N(0,
#'    \Theta_V) \\
#'   W_i \overset{i.i.d.}{\sim} \mathrm{gamma} \big( (\nu + 1)/2, \nu/2 \big)
...
#'
#' \code{nu} is degrees of freedom for Bessel distributions. If \code{nu == 1}
#'  then the random variables are Laplace distributions. If \code{nu} goes to
#'  infinity then the random variables converge to Gaussian distributions.
#'
...
#' @import GIGrvg
#' @export
pSUN_LogitBessit_Gibbs = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste("pSUN_LogitBessit_Gibbs: start time at ", Sys.time(), sep = ""))
  }
  ...
  for (iburn in 1:burn) {
    ...
    ### update W
    W = apply(matrix(Z^2), 1, FUN = function(xappl) {
      tryCatch(
        expr = GIGrvg::rgig(n = 1, lambda = 0.5 * nu, chi = xappl, psi = nu),
        error = function(err) {
          GIGrvg::rgig(
            n = 1, lambda = 0.5 * nu,
    ...
    decompVar_zGNeps = tryCatch(
      ...
      error = function(err){ # if chol fails then use svd
        ...
        return(backsolve(
          r = chol(diag(1 / W, nrow = d) + crossprod(A, A / sqrt(V)) +
                     diag(sqrt(.Machine$double.eps), d)),
          x = diag(1, nrow = d)
        ))
      }
  ...
  if (!is.infinite(verbose)) {
    print(paste("pSUN_LogitBessit_Gibbs: end time at ", Sys.time(), sep = ""))
  }
