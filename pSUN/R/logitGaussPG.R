#' @title Logit Model with Gaussian Prior: Polya-Gamma Gibbs Sampler

#' @description
#' Polya-Gamma Gibbs sampler for the logit model with Gaussian prior. It is
#'  used the code from package \pkg{BayesLogit} with slight modifications.

#' @usage 
#' logitGaussPG = function(nsim, X, y, mu, Sigma, x0 = NULL, burn = 1,
#'                          thin = 1, verbose = +Inf)
...
#' }
#'
#' The algorithm described in Polson et al. (2013) is used.
#'
...
logitGaussPG = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitGaussPG: start time at ", Sys.time(), sep = ""))
  }
  ...
  ### compute t(X) %*% (y - 1/2) + invSigma %*% mu
  tXyOh_invSigmaMu = t(X) %*% (y - 0.5) + invSigma %*% mu
  ...
  for (iburn in 1:burn) {
    ...
    decompV_omega = chol(t(X) %*% (X * omega) + invSigma)
    ...
    ### sample beta
    beta = V_omega %*% tXyOh_invSigmaMu + decompV_omega %*% rnorm(p)
  ...
  for (insim in 1:nsim) {
    ...
    for(ithin in 1:thin){
      ...
      ### decomposition of variance-covariance matrix of beta given omega and y
      #   (V_omega)
      # upper Cholesky factor of the inverse of V_omega
      decompV_omega = chol(t(X) %*% (X * omega) + invSigma)
      # inverse of the upper Cholesky factor of the inverse of V_omega
      decompV_omega = backsolve(
        r = decompV_omega,
        x = diag(1, nrow = p)
      )
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitGaussPG: end time at ", Sys.time(), sep = ""))
  }
