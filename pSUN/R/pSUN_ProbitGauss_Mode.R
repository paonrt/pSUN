################################################################################
#
# Perturbed Unified Skew Normal (pSUN), Probit Gauss: Mode Finder
#
################################################################################
...
############### function
pSUN_ProbitGauss_Mode = function(
  ...
  ### compute arguments of probit CDF
  argProbitCDF = c(A %*% betaTilde + b)

  ### compute gradient of Probit CDF (it is equal to ratio of PDF over CDF)
  gradProbitCDF = stdGaussPDFoCDF(argProbitCDF)

  ### compute gradient of Lambda
  gradLambda = t(A) %*% gradProbitCDF
  ...
  while (TRUE) {
    ...
    ### compute negative Hessian of Probit CDF
    negHessProbitCDF = gradProbitCDF * (argProbitCDF + gradProbitCDF)

    ### compute decomposition of negative Hessian matrix (M^T)
    decompNegHes = t(sqrt(negHessProbitCDF) * A)
    ...
    ### update arguments of probit CDF
    argProbitCDF = c(A %*% betaTilde + b)

    ### update gradient of Probit CDF (it is equal to ratio of PDF over CDF)
    gradProbitCDF = stdGaussPDFoCDF(argProbitCDF)
  ...
  if (details) {
    ...
    approxLogNormConst = - 0.5 * crossprod(a_ras, betaTilde) +
      sum(pnorm(argProbitCDF, log.p = TRUE)) - sum(log(diag(cholL_ras))) +
...
if (FALSE) { # appendix script is not run
  ...
  # debugging
  {
    ...
    source(paste(.libPaths()[1], "/myFUN/pSUN_ProbitGauss_Mode.R", sep = ""))
    set.seed(1984)
    d = 500
    m = 30
  ...
  # test CancerSAGE
  {
    ...
    t0 = Sys.time()
    res_pSUN_mode = pSUN_ProbitGauss_Mode(A, b, xi, Omega)
    tn = Sys.time()
    print(difftime(tn, t0, units = "sec"))
