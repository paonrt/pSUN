#' @title Logit Model with Student-t Prior: Ultimate Polya-Gamma Gibbs Sampler

#' @description
#' Ultimate Polya-Gamma Gibbs sampler for the logit model with Student-t prior.
#'  It is used the code from package \pkg{UPG} with slight modifications.

#' @usage
#' logitStudentUPG = function(nsim, X, y, nu, Sigma, x0 = NULL, burn = 1,
#'                            thin = 1, verbose = +Inf)
...
logitStudentUPG = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitStudentUPG: start time at ", Sys.time(), sep = ""))
  }
  ...
  for (iburn in 1:burn) {
    ...
    W = 1 / rgamma(
      n = 1, shape = 0.5 * (nu + p),
      rate = 0.5 * (nu + crossprod(invCholSigma %*% beta))
    )
  ...
  if (!is.infinite(verbose)) {
    print(paste("logitStudentUPG: end time at ", Sys.time(), sep = ""))
  }
