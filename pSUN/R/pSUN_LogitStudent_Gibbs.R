#' @title Perturbed Unified Skew Normal Logit Student-t: Gibbs Sampler

#' @description
#' Gibbs Sampler for a perturbed unified skew normal (pSUN) logit Student-t
#'  distribution.

#' @usage
#' pSUN_LogitStudent_Gibbs = function(nsim, A, b, nu, xi, Omega, x0 = NULL,
#'                                    burn = 1, thin = 1, plim = 1 / nsim,
#'                                    verbose = +Inf)

#' @details
#' It is said \eqn{X} follows a pSUN logit Student-t distribution if
#'  \deqn{
...
#'  infinity then the random variables converge to Gaussian distribution.
#'
...
#' @export
pSUN_LogitStudent_Gibbs = function(
  ...
  if (!is.infinite(verbose)) {
    print(paste(
      "pSUN_LogitStudent_Gibbs: start time at ", Sys.time(), sep = ""
    ))
  }
  ...
  for (iburn in 1:burn) {
    ...
    Sigma_eps = diag(V) + W * AOmegaBartA
    ...
    decompVar_zGNeps = tryCatch(
      ...
      error = function(err){ # if chol fails then use svd
        ...
        return(backsolve(
          r = chol(invOmegaBar / W + crossprod(A, A / sqrt(V)) +
                     diag(sqrt(.Machine$double.eps), d)),
          x = diag(1, nrow = d)
        ))
      }
  ...
  if (!is.infinite(verbose)) {
    print(paste("pSUN_LogitStudent_Gibbs: end time at ", Sys.time(), sep = ""))
  }
